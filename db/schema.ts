...
export const nodes = pgTable('nodes', {
  ...
  type: text('type', { enum: ['Idea', 'Assumption', 'Plan', 'Test', 'Result', 'Decision', 'Lesson'] }).notNull(),
  text: text('text').notNull(),
  rationale: text('rationale'),
  constraints: jsonb('constraints').$type<string[]>(),
  metric: text('metric'),
  threshold: jsonb('threshold').$type<{ pass?: number; fail?: number }>(),
  ifThenPlan: jsonb('if_then_plan').$type<{ date?: string; time?: string; place?: string; action?: string }>(),
...
export const branches = pgTable('branches', {
  id: uuid('id').primaryKey().defaultRandom(),
  state: text('state', { 
    enum: ['Seeded', 'Diverging', 'Clarifying', 'Planning', 'Testing', 'Reviewing', 'Stalled', 'Action crisis', 'Archived'] 
  }).notNull().default('Seeded'),
